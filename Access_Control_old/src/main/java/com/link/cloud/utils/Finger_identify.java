...
public class Finger_identify {
   ...
   public static String Finger_identify (LockActivity activty, byte[] img){
       ...
       Cursor cursor;
       String sql;
       sql = "select UID,FINGERMODEL from PERSON where UID in(select USER_ID from SIGN_USER)" ;
       ...
       if(feature.length>0) {
           ...
           if (score[0]<IDENTIFY_SCORE_THRESHOLD){
               i=0;
               Cursor cursor1;
               sql="select t2.UID,t2.FINGERMODEL from  PERSON t2";
               ...
               while (cursor1.moveToNext()){
                   int nameColumnIndex = cursor1.getColumnIndex("FINGERMODEL");
