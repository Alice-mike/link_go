 ...
 */
public class BaseApplication extends MultiDexApplication  implements GetDeviceIDContract.VersoinUpdate,DownloadFeature.download,CabinetNumberContract.cabinetNumber{
...
//        GcmRegister.register(applicationContext, "send_id", "application_id"); // 接入FCM/GCM初始化推送
    ...
    public void getDeviceSuccess(DeviceData deviceData) {
       Logger.e("BaseApplication+devicedate"+deviceData.getDeviceData().getDeviceId()+"numberType"+deviceData.getDeviceData().getNumberType());
            ...
            new Thread(new Runnable() {
                @Override
                public void run() {
                    cabinetNumberContract.cabinetNumber(deviceData.getDeviceData().getDeviceId());
                }
            }).start();
            final CloudPushService pushService = PushServiceFactory.getCloudPushService();
    ...
    public void cabinetNumberSuccess(CabinetNumberData cabinetNumberData) {
        ...
        Toast.makeText(getContext(),"下载柜号列表成功",Toast.LENGTH_SHORT).show();
    ...
    public static void setConsoleText(String text) {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                feature.download(pushMessage.getMessageId(),pushMessage.getAppid(),pushMessage.getShopId(),deviceID,pushMessage.getUid());
            }
        }).start();
    }
