...
public class ThirdActivity extends BaseAppCompatActivity implements IsopenCabinet.isopen{
        ...
        public void handleMessage(android.os.Message msg) {
            switch (msg.what) {
                ...
                case 1:
                    ...
                    Logger.e("ThirdFragment=============pos[0]="+pos[0]);
                    if (users.size()>0) {
                        String uid = users.get(0).getUid();
                        Logger.e("ThirdFragment=============="+pos[0]+"==uid="+uid);
                        new Thread(new Runnable() {
                            @Override
                            public void run() {
                                isopenCabinet.isopen(2,deviceId,uid,"vein");
                            }
                        }).start();
                    }else {
                        text_error.setText("没找到该会员数据");
                    }
//                    matchVeinTaskContract.signedMember("pmljt8z",uid,"vein");
                    break;
...
//                Logger.e("BindActivty===========state"+state);
                if (state != 0) {
                    ...
                    if (featuer!=null) {
                        ...
                        Logger.e("ThirdFragment_count"+"===========featuer.length"+featuer.length/3352);
                    }else {
                        if (handler != null) {
    ...
    public void isopenSuccess(Lockdata resultResponse) {
        ...
        cabinetRecord.setCabinetStating("取件退柜");
