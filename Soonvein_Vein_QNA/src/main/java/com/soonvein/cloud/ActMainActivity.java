...
import com.soonvein.cloud.fragment.LoginFragment;
...
import com.soonvein.cloud.fragment.PayMainFragment;
import com.soonvein.cloud.utils.Utils;
import com.soonvein.cloud.view.NoScrollViewPager;
import com.soonvein.cloud.view.TabViewPager;
...
//        mediaPlayer1=MediaPlayer.create(this,R.raw.error_password);
    ...
    protected void initViews(Bundle savedInstanceState) {
        ...
        filter.addAction(SdkMain.ACTION_VEIN_DEVICE_PERMISSION);//USB设备断开连接广播
...
//                    container.setAdapter(mPageAdapter);
    ...
    @Override
    public void run() {
        SdkMain.getInstance().checkState();
    ...
    public static final BroadcastReceiver mUsbReceiver = new BroadcastReceiver() {
        public void onReceive(Context context, Intent intent) {
            ...
            if (device != null && 0x0481 == device.getVendorId() && 0x5641 == device.getProductId()) {
                switch (action) {
                    case SdkMain.ACTION_VEIN_DEVICE_PERMISSION:
                        SdkMain.getInstance().setUsbDevice(device, intent.getBooleanExtra(UsbManager.EXTRA_PERMISSION_GRANTED, false));
                        break;
                    case UsbManager.ACTION_USB_DEVICE_ATTACHED:
                        SdkMain.getInstance().setUsbDevice(device);
                        break;
                    case UsbManager.ACTION_USB_DEVICE_DETACHED:
                        ToastUtils.show(context, "USB设备连接已断开", ToastUtils.LENGTH_LONG);
                        //收到USB设备拔出广播
                        SdkMain sdkMain = SdkMain.getInstance();
                        if (device != null && sdkMain.getUsbDevice() != null
                                && device.getDeviceId() == sdkMain.getUsbDevice().getDeviceId()) {
                            //用户授权使用的USB设备拔掉
                            SdkMain.getInstance().deviceDetached();
                        }
                        break;
                }
            }
